...
public class CarShopStorageServiceImpl implements CarShopStorageService {
	...
	public void sellCar(Car car) throws RequestedCarNotFoundException {
		...
		for (Car searchedCar: carsOfSameType) {
			if ((searchedCar.getPrice() == car.getPrice())
					&& (searchedCar.getColor() == car.getColor())
					&& (searchedCar.getConstructionYear() == car.getConstructionYear()))  {
				CarShopStorage.getInstancce().getCars().remove(searchedCar);
			} else {
				throw new RequestedCarNotFoundException("Car was't found in the storage.");
			}
		}
	}
}
